/**
 * \mainpage Biblioteka LSM6DS3 v.1.0.0
 * 
 * Biblioteka umożliwiająca uruchomienie akcelerometru, żyroskopu znajdujacego się w sensorze LSM6DS3.
 * Sensor ten znajduje się na płytce Arduino nano 33 IoT.
 * - Plik Lsm6ds3IoT.cpp zawiera definicje funkcji
 * - Plik Lsm6ds3IoT.h zawiera deklaracje funkcji
 */
 ...
 * @brief Plik z definicjami funkcji obsługujących sensor LSM6DS3
 * @version 0.1
 * @date 2021-06-24
 ***************************************************************************
 */
 ...
 * @return uint16_t Funkcja zwraca zawrtość rejestru podanego w parametrze RegisterAddr
 */
...
void LSM6DS3::Accelerometer_XYZ_read_value(AccelOutput_t *OutData, SensorSettings_t *range){
  ...
  double sensitivity = 0; /**< zmienna lokalna do przetrzymania rozdzielczosci potrzebnej do przeskalowania wyników odczytachy z rejestrów */
  ...
  /** Sprawdzenie czy w STATUS_REG bit nr.0 jest ustawiony na 1, jezeli tak to znaczy że jest nowa wartosć do odczytania w wyjsciowych rejestrach akcelerometry.
   */
...
void LSM6DS3::Temperature_read_value(TempOutput_t *OutData) {
  ...
  /** Sprawdzenie czy w STATUS_REG bit nr.2 jest ustawiony na 1, jezeli tak to znaczy że jest nowa wartosć do odczytania w wyjsciowych rejestrach termometru.
   */
